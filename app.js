```javascript
var UNITS=[{n:"Environments"},{n:"Interactions"},{n:"Heroes"},{n:"Events That Matter"},{
var TABS=["Overview","I Do","We Do","You Do","Scaffold and Diff","Data Tracker","Daily P
var WC={"ELP 1":"e1","ELP 2":"e2","ELP 3":"e3","ELP 4":"e4","ELP 5-6":"e56"};
var W=LESSONS;var mode="push",sel=null,curTab=0,uW;
window.addEventListener("DOMContentLoaded",function(){
 ...
 document.getElementById("sbtog").addEventListener("click",function(){document.getEleme
 document.getElementById("pbtn").addEventListener("click",function(){window.print();})
 document.getElementById("srch").addEventListener("input",function(){buildNav(this.valu
 ...
 var a=loadDT();
 a[parseInt(el.dataset.i)].notes=el.value; saveDT(a);
 }
 });
});
function buildNav(q){
 ...
 hdr.className="uhdr";
 hdr.dataset.ui=ui;
 hdr.innerHTML="<div class='un'>"+(ui+1)+"</div><div class='uname'>Unit "+(ui+1)+": "
 div.appendChild(hdr);
...
function setMode(m){
 mode=m; document.getElementById("bp").classList.toggle("on",m==="push");
...
function switchTab(i){
 curTab=i;
 document.querySelectorAll(".tab").forEach(function(t){t.classList.toggle("on",parseInt
...
function widaHtml(wd){
 ...
 var cl=WC[lv]||"e56";
 h+="<div class='wrow'><span class='wb "+cl+"'>"+safe(lv)+"</span><span class='wtx'>"
 });
 return h;
}
function vocabHtml(d){
 ...
 h+=d.v3.map(function(v){return "<span class='vc t3'>"+safe(v)+"</span>";}).join(""); h+="</div></div>";
 return h;
}
function diffHtml(df){
 var h="<div class='dr'>";
 h+="<div class='drow'><span class='dlv d0'>Below</span><span class='dtx'>"+safe(df.b)+
 h+="<div class='drow'><span class='dlv d1'>On Level</span><span class='dtx'>"+safe(df
 h+="<div class='drow'><span class='dlv d2'>Above</span><span class='dtx'>"+safe(df.a)+
 h+="<div class='drow'><span class='dlv d3'>ELP Mix</span><span class='dtx'>"+safe(df.e
 h+="</div>";
 return h;
}
function tryBox(tp){
 return "<div class='try'><div class='try-i'>&#10024;</div><div><div class='try-l'>Try 
}
...
function genOverview(d){
 var h="<div class='ob'><div class='olb'>Content Objective</div><div class='stx'>"+safe
 h+="<div class='ob lang'><div class='olb'>Language Objective</div><div class='stx'>"+s
...
function genIDo(d){
 ...
 h+=say("SAY:","Today we are going to "+safe(d.co)+" Our language goal is: <em>"+safe(d
 h+=doBox("SETUP:","Distribute <strong>"+safe(d.mn[0])+"</strong>. Write the language o
 h+=doBox("VOCAB PREVIEW:","Preview Tier 2 words: <strong>"+d.v2.map(safe).join(" - ")+
 h+=say("THINK ALOUD:","Watch me model this step by step. I am going to THINK OUT LOUD
 h+=doBox("MODEL 1:","Complete one full example using "+safe(d.mn[0])+". Narrate every 
 h+=say("SAY:","Notice what I did: found evidence first, used a vocabulary word, comple
 h+=doBox("MODEL 2:","A second example with different evidence and a different color ma h+=elpBox("ELP 1-2 SUPPORT:",d.sc[0]+" Ensure ELP 1-2 students have a completed model 
 h+=say("CHECK:","Thumbs up if you understand. Thumbs sideways if you need to see it on
 h+=tryBox(d.tp);
 return h;
}
function genWeDo(d){
 ...
 h+=doBox("GUIDED TASK:","Begin "+safe(d.ac)+" together as a class. Complete the first 
 h+=say("PARTNER TALK:","Turn to your partner. Use the language objective frame to star
 h+=doBox("WAIT TIME:","Give genuine 45 to 60 second think time. Circulate and note who
 h+=elpBox("PROMPT ELP 1-2:",wk[0]?d.wd[wk[0]]:"Accept pointing, gesturing, and one-wor
 h+=elpBox("PROMPT ELP 3-4:",wk[2]?d.wd[wk[2]]:"Use sentence starters to support.");
 h+=elpBox("PROMPT ELP 5-6:",wk[4]?d.wd[wk[4]]:(wk[3]?d.wd[wk[3]]:"Extend thinking with
 h+=doBox("PRAISE:","I love how you used the sentence frame. That is exactly the right 
 h+=doBox("SHARE OUT:","Call on 2 to 3 students. Restate using full academic language. 
 h+=say("TRANSITION:","You worked so well together. Now I want to see you try this on y
 return h;
}
function genYouDo(d){
 ...
 h+=say("REMIND:","If you are stuck: check your language objective frame, check the wor
 ...
 h+=doBox("MONITOR:","Circulate with the data tracker open. Note who uses the frame ind
 h+=say("CLOSE:","Let us share out. There is no wrong answer if you used evidence and t
 h+=tryBox(d.tp);
 return h;
}
function genSD(d){
 var h="<div class='sh'>Manipulatives and Materials</div><div class='mchips'>";
 h+=d.mn.map(function(m){return "<span class='mch'>&#128230; "+safe(m)+"</span>";}).joi
...
function genDT(){
 ...
 var h="<div class='dth'><div class='dtt'>Data Tracker<br><small style='font-size:.63re
 h+=(mode==="push"?"Push-In":"Pull-Out")+" - "+safe(W[sel].t)+"</small></div>";
 h+="<div style='display:flex;gap:6px;flex-wrap:wrap'><button class='btn' data-action=
 h+="<button class='btno' data-action='printDT'>&#128424; Print Data</button></div></di
 h+="<div class='sums'><div class='sb2'><div class='snum' id='sumT'>"+tot+"</div><div c
 h+="<div class='sb2' style='background:#f0fdf4;border-color:#bbf7d0'><div class='snum
 h+="<div class='sb2' style='background:#fffbeb;border-color:#fcd34d'><div class='snum
 h+="<div class='sb2' style='background:#fff7ed;border-color:#fed7aa'><div class='snum
 h+="<div class='aform' id='addFm'><div class='frow'><input class='fi' id='sn' placehol
 h+="<select class='fsel' id='sp'><option value=''>ELP Level</option><option>ELP 1</opt
 h+="<div class='frow'><button class='btn g' data-action='addS'>Add Student</button><bu
 if(arr.length){
 h+="<div style='overflow-x:auto'><table class='dttable'><thead><tr><th>Student</th><
 arr.forEach(function(s,i){
 h+="<tr><td><strong>"+safe(s.name)+"</strong></td><td><span class='vc t2'>"+safe(s
 h+="<td><label class='cbx'><input type='checkbox' class='dtck' data-i='"+i+"' data
 h+="<td><label class='cbx'><input type='checkbox' class='dtck' data-i='"+i+"' data
 h+="<td><label class='cbx'><input type='checkbox' class='dtck' data-i='"+i+"' data
 h+="<td><textarea class='ni dtnote' rows='2' data-i='"+i+"'>"+safe(s.notes||"")+"<
 h+="<td><button class='delb' data-action='del' data-idx='"+i+"'>&#10005;</button><
 });
 h+="</tbody></table></div>";
 }else{
...
function refreshSummary(){
 var arr=loadDT();
 var both=arr.filter(function(s){return s.co&&s.lo;}).length;
 var part=arr.filter(function(s){return (s.co||s.lo)&&!(s.co&&s.lo);}).length; var none=arr.filter(function(s){return !s.co&&!s.lo;}).length;
...
function printDT(){
 ...
 var rows=arr.map(function(s){ var coC=s.co?"green":"#ccc";
 ...
 html+="table{width:100%;border-collapse:collapse}th,td{border:1px solid #ccc;padding:5
 html+="th{background:#ede9fe;font-size:10px;text-transform:uppercase}</style></head><b
 html+="<h2>Data Tracker: "+safe(l.t)+"</h2><p>Unit "+(l.u+1)+": "+safe(UNITS[l.u].n)+"
 html+=(mode==="push"?"Push-In":"Pull-Out")+" - "+new Date().toLocaleDateString()+"</p>
 html+="<table><thead><tr><th>Student</th><th>ELP</th><th>CO Met</th><th>LO Met</th><th
...
function genDailyPlan(d,l){
 ...
 h+="<div class='dpbody'><div class='dplabel'>"+n+"</div><div class='dptext'>"+c+"</d
 h+="</div>";
 return h;
 ...
 var hd="<div class='dpintro'><strong>30-Minute Daily Breakdown</strong> - <em>"+tt+"</
 hd+="<span style='color:#7c3aed'>CO:</span> "+co+" &nbsp;|&nbsp; <span style='color:#0
 var d1rows=R("5 min","Hook",tp);
 d1rows+=R("8 min","Vocabulary Preview","Tier 2 words: <strong>"+v2+"</strong>. Say, cl
 d1rows+=R("10 min","Read Aloud","Read <em>"+tx+"</em> aloud. Pause to think aloud 2 to
 d1rows+=R("5 min","Turn and Talk","What is this text mostly about? Use 1 vocabulary wo
 d1rows+=R("2 min","Exit Ticket","Write or draw one new word and one thing noticed abou
 var d1=C("Day 1 - Monday","Hook and Vocabulary Preview","c1",d1rows);
 var d2rows=R("3 min","Warm-Up Review","Quick vocab flash: thumbs up if you know the me
 d2rows+=R("12 min","I Do - Two Full Models","Distribute "+mn+". Model 1: think aloud e
 d2rows+=R("10 min","We Do - Guided Attempt","Complete the first item together. ELP 1-2
 d2rows+=R("5 min","Partner Practice","Partners complete one more item. Give specific, 
 var d2=C("Day 2 - Tuesday","I Do - Teacher Models","c2",d2rows);
 var d3rows=R("3 min","Review","One student recalls the content objective. Class reads 
 d3rows+=R("5 min","We Do Round 2","One more guided example. Cold-call with 15-second w
 d3rows+=R("15 min","You Do",ac+" Below: "+dfb+" On Level: "+dfo+" Above: "+dfa+" Circu
 d3rows+=R("5 min","Share Out","Call on 2 to 3 students. Echo the sentence frame as a c
 ...
 var d4rows=R("5 min","Group Students","Sort Day 3 exit tickets into mastered both, par
 d4rows+=R("15 min","Differentiated Practice","Small group ELP 1-2: reteach with "+mn+"
 d4rows+=R("8 min","Data Tracker","Record CO, LO, and Key Skill for each student. Add n
 ...
 var d5rows=R("5 min","Vocabulary Game","Say the definition, students race to point to 
 d5rows+=R("10 min","Student Share Out","Share completed "+mn+". ELP 1-2: point to best
 d5rows+=R("10 min","Exit Ticket Assessment","Content objective: "+co+". Language objec
 d5rows+=R("5 min","Celebrate","Specific praise by name. Students write one thing they 
 var d5=C("Day 5 - Friday","Share Out and Celebrate","c5",d5rows);
 return hd+d1+d2+d3+d4+d5;
}
function renderLesson(){ var l=W[sel];
 var u=UNITS[l.u];
 ...
 return "<button class='tab"+(i===curTab?" on":"")+"' data-tab='"+i+"'>"+safe(t)+"</b
 }).join("");
 var contents=[genOverview(d),genIDo(d),genWeDo(d),genYouDo(d),genSD(d),genDT(),genDail
 ...
 hdr+="<div class='ctags'><span class='ctag'>"+safe(l.x)+"</span><span class='ctag'>"+s
 var stds="<div class='stds'><span class='slbl'>VDOE SOL</span>";
 stds+=l.s.map(function(s){return "<span class='chip'>"+safe(s)+"</span>";}).join("");
 stds+="<span class='chip ci'>CIP: "+safe(l.c)+"</span><span class='chip cv'>Vista: "+s
 document.getElementById("cw").innerHTML=hdr+stds+"<div class='tabs'>"+tabsHtml+"</div>
}
